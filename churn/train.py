...
oot = df.loc[df['dtRef'] == df['dtRef'].max()].copy()
df_train = df.loc[df['dtRef'] < df['dtRef'].max()].copy()
...
#%%
best_features = features_importance[features_importance['acum.'] < 0.96]['index'].tolist()
best_features
...
reg.fit(X=X_train_transform, y=y_train)
...
auc_train = metrics.roc_auc_score(y_true=y_train, y_score=y_train_proba)
